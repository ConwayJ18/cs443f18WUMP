...
public class HUMPclient {
    ...
    static public void main(String args[]) {
        ...
        String filename = "vanilla"; //delay not working properly
       ...
       if (args.length > 3) {
        ...
        while(true)
        {
                ...
                {
                    //retransmit of previous packet here
                    try
                    {
                      s.send(lastSent);
                    }
        ...
        byte[] replybuf = replyDG.getData();
      	...
      	// check that replybuf looks like a HANDOFF
        if(wumppkt.opcode(replybuf) == wumppkt.HANDOFFop && replyDG.getPort() == destport)
        {
            // If so: wumppkt.HANDOFF handoff = new wumppkt.HANDOFF(replybuf);
            ...
            //expected_block++; //Take this out in HUMP?
